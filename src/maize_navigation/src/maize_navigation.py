...
class FieldRobotNavigator:
    def __init__(self):
        ...
        self.row_index_pub = rospy.Publisher("/row_index", Int32, queue_size=10)
        self.row_dir_pub = rospy.Publisher("/row_dir", String, queue_size=10)
        ...
        self.pid_controller = PID(kp=2.25, ki=0.01, kd=0.75, integral_limit=1.0)#simu: (kp=2.25, ki=0.01, kd=0.75, integral_limit=1.0)
        
        # Polyfit parameters
        # self.poly_min_dist_req = 0.45 # wie lang muss ein Polynom sein, um als gültig zu gelten
        # self.polydist_to_robot = 0.05 # was ist der minimale Abstand, ab dem ein Polynom anfangen kann
        # self.needed_points_for_fit = 20 # wie viele Punkte müssen erkannt werden, um ein Linefit durchzuführen

        self.poly_min_dist_req = rospy.get_param("min_poly_length")
        self.polydist_to_robot = rospy.get_param("min_poly_dist")
        self.needed_points_for_fit = rospy.get_param("min_points")
    ...
    def drive_in_row(self):
        ...
        #for simu validation_x = 5 * cycle_time * self.last_linear_speed
        self.last_linear_speed = 0
        ...
        if len(left_points) < 3 and len(right_points) < 3:
            ...
            if (
                ...
                if (
                    ...
                    self.publish_poly_marker(left_poly_coeffs, "left_poly", min_left_x, max_left_x)
                ...
                if (
                    ...
                    self.publish_poly_marker(right_poly_coeffs, "right_poly", min_right_x, max_right_x)
            ...
            rospy.loginfo("Publishing to cmd_vel_raw: %s", cmd_vel)
    ...
    def turn_to_row(self):
        ...
        if self.angle is None:
            rospy.logwarn("Angle not yet received.")
            return
        if np.abs(self.angle) < 5:
            cmd_vel = Twist()
        ...
        self.cmd_vel_pub.publish(cmd_vel)
        self.row_index_pub.publish(self.driven_row)
        self.row_dir_pub.publish(self.pattern[self.driven_row - 1][1])
